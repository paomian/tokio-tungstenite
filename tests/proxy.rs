...
async fn http_connect_without_auth() {
    ...
    let mut stream =
        open_tunnel(&tunnel_request("wss://ws.example.org/socket"), proxy, false).await.unwrap();
...
async fn socks5h_with_auth_and_domain_resolution() {
    ...
    let server = task::spawn(async move {
        ...
        socket.write_all(&[0x05, 0x00, 0x00, 0x01, 127, 0, 0, 1, 0x10, 0x00]).await.unwrap();
    ...
    let mut stream =
        open_tunnel(&tunnel_request("wss://ws.example.org/"), proxy, false).await.unwrap();
    stream.write_all(b"ping").await.unwrap();
    let mut buf = [0u8; 4];
    stream.read_exact(&mut buf).await.unwrap();
    assert_eq!(&buf, b"pong");
...
async fn socks5_no_auth_ip_target() {
    ...
    let server = task::spawn(async move {
        ...
        socket.write_all(&[0x05, 0x00, 0x00, 0x01, 127, 0, 0, 1, 0x23, 0x45]).await.unwrap();
    });

    let proxy = Proxy::new(format!("socks5://{}", proxy_addr).parse().unwrap());
    let request = http::Request::builder().uri("ws://127.0.0.1:9001").body(()).unwrap();
